...
- Server status (TCP-probed 2026-07-03): ZEC `zec-cce-1/2.coinomi.net:5028` OPEN but **STALE — DO NOT USE**; ZCL `zcl-cce-1/2.coinomi.net:5015` OPEN but **UNVERIFIABLE**; YEC — NO public ElectrumX found (`electrum1/2.ycash.xyz` closed; `lite.ycash.xyz:443` is lightwalletd, not usable). YEC is GATED on Task 10 infra.
- **Task 1 PROTOCOL-VERIFY RESULTS (2026-07-03, Electrum protocol handshake, not just TCP):**
  - ZEC `zec-cce-1` and `zec-cce-2` `.coinomi.net:5028`: **PLAINTEXT (no SSL)**, ElectrumX 1.16.0, protocol 1.4, `blockchain.relayfee`=0.001. Reported height **2,638,773** on BOTH nodes (advancing slowly). ❌ **STALE by ~761K blocks**: true Zcash mainnet tip = **3,399,813** as of 2026-07-03 21:12 UTC (blockchair `best_block_time` = today; independently corroborated — NU6.2 activated at mainnet height 3,364,600 in June 2026, Ironwood targets 3,417,100 late July 2026). The coinomi ZEC server is ~1.8 years behind and predates NU6.2 → cannot see recent UTXOs and cannot relay current-branch-ID txs. **Phase A/B blocked on data source.**
  - ZCL `zcl-cce-1`/`-2` `.coinomi.net:5015`: **PLAINTEXT (no SSL)**, ElectrumX 1.16.0, protocol 1.4, `blockchain.relayfee`=0.0001. Reported height **5,938,713**. ⚠️ **UNVERIFIABLE / INCONSISTENT**: ZelCore ZCL explorer (`explorer.zcl.zelcore.io`) shows tip ≈ **3,169,093** — the coinomi server is ~2.77M blocks *ahead* of that explorer, which is impossible for a healthy node on the same chain. Cannot confirm which chain is canonical/current → **Phase C gated per Task 1 Step 2.**
